...
use crate::{
    conflict::{ConflictResolution, ConflictSolver, UnsolvableConflict},
    format::{
...
pub enum RepositoryError {
    ...
    RebaseFailed { snapshot: SnapshotId, conflicts: Vec<UnsolvableConflict> },
    ...
    pub async fn rebase(
        ...
        if ref_data.snapshot != self.snapshot_id {
            ...
            for snap_id in new_commits.into_iter().rev() {
                ...
                match solver.solve(&tx_log, &repo, changeset).await? {
                    ConflictResolution::Patched(patched_changeset) => {
                        changeset = patched_changeset;
                    }
                    ConflictResolution::Failure { reason, .. } => {
...
async fn distributed_flush<I: IntoIterator<Item = ChangeSet>>(
    ...
    let tx_log = TransactionLog::from(&change_set);
...
mod tests {
    ...
    use crate::{
        conflict::{BasicConflictSolver, NoFastForward, VersionSelection},
    ...
    async fn test_rebase_without_fast_forward() -> Result<(), Box<dyn Error>> {
        ...
        {
            let result = repo2.rebase(&NoFastForward(), "main").await;
            assert!(matches!(
                result,
                Err(RepositoryError::RebaseFailed { snapshot, conflicts, }) if snapshot == conflicting_snap && conflicts == vec![UnsolvableConflict::NoFastForwardConfigured]));
        } else {
    ...
    async fn test_rebase_fast_forwarding_over_chunk_writes() -> Result<(), Box<dyn Error>>
    {
        ...
        repo2
            .set_chunk_ref(
                new_array_path.clone(),
                ChunkIndices(vec![1]),
                Some(ChunkPayload::Inline("overriden".into())),
            )
            .await?;
            ...
            repo2.commit("main", "write one chunk with repo2", None).await
        {
            ...
            assert!(matches!(
                ...
                Err(RepositoryError::RebaseFailed { snapshot, conflicts, })
                    ...
                    conflicts == vec![UnsolvableConflict::WriteToWrittenChunk(expected_conflicts)]));
        } else {
            panic!("Bad test, it should conflict")
        }
        ...
        // let's try to create a new commit, that conflicts with the previous one and writes
        // to the same chunk, recovering with "Ours" policy
        let mut repo2 =
            Repository::update(Arc::clone(&storage), array_created_snap.clone()).build();
        repo2
            .set_chunk_ref(
            ...
            repo2.commit("main", "write one chunk with repo2", None).await
        {
            let solver = BasicConflictSolver {
                on_chunk_write_conflicts: VersionSelection::Ours,
                ..Default::default()
            };
            ...
            assert_eq!(data, Some(ChunkPayload::Inline("overriden".into())));
            let commits = repo2.ancestry().await?.try_collect::<Vec<_>>().await?;
            assert_eq!(commits[0].message, "after conflict");
            assert_eq!(commits[1].message, "write two chunks with repo 1");
        } else {
        ...
        // let's try to create a new commit, that conflicts with the previous one and writes
        // to the same chunk, recovering with "Theirs" policy
        let mut repo2 =
            Repository::update(Arc::clone(&storage), array_created_snap.clone()).build();
        repo2
            .set_chunk_ref(
        ...
        {
            let solver = BasicConflictSolver {
                on_chunk_write_conflicts: VersionSelection::Theirs,
                ..Default::default()
            };
